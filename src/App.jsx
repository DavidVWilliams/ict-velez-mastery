...
export default function App() {
  ...
  // DIRECT FRONTEND API CALLS USING VITE_GEMINI_API_KEY
  const callGemini = async (promptText) => {
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
    if (!apiKey) { setAiResponse("System Error: VITE_GEMINI_API_KEY is not defined in your Vercel Environment Variables."); return; }
  ...
  const callLessonGemini = async (lessonTitle) => {
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
    if (!apiKey) { setLessonAiResponse("System Error: VITE_GEMINI_API_KEY is not defined in your Vercel Environment Variables."); return; }
  ...
  // Note: All video links now point directly to The Inner Circle Trader's official 2022 Mentorship playlist
  const OFFICIAL_PLAYLIST_URL = "https://www.youtube.com/playlist?list=PLVgHx4Z63paYiFGQ56PjTF1PGePL3r69s";

  const fleshedOutEpisodes = [
    {
      id: "ep1",
      title: "Episode 1: The Magnet (Liquidity)",
      videoUrl: OFFICIAL_PLAYLIST_URL,
  ...
  // Dynamically generate episodes 8-41 to prevent code truncation
  const remainingEpisodes = Array.from({ length: 34 }, (_, i) => ({
    id: `ep${i + 8}`,
  ...
  const fullCourseData = [...courseData, ...remainingEpisodes];
            ...
            <div className="w-full lg:w-1/4 flex flex-col space-y-6 sticky top-24">
              <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 shadow-xl">
                ...
                <div className="space-y-4">
                  ...
                  />
                  <button 
                    onClick={() => callLessonGemini(fullCourseData.find(l => l.id === activeLessonId)?.title)} 
