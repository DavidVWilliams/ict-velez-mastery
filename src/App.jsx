...
export default function App() {
  ...
  const callGemini = async (promptText) => {
    ...
    try {
      ...
      setAiResponse("API Connection Error: Ensure /api/gemini.js is deployed correctly on Vercel. " + err.message); 
    } finally { 
      setLoadingAi(false); 
    }
  };
  ...
  const callLessonGemini = async (lessonTitle) => {
    ...
    try {
      ...
      setLessonAiResponse("API Connection Error: Ensure /api/gemini.js is deployed correctly on Vercel. " + err.message); 
    } finally { 
  ...
  const episodeTitles = [
    ...
    "Episode 4: The Safety Filter (Oliver Velez 200 SMA)", "Episode 5: Intra-day Order Flow", "Episode 6: Daily Range & Market Structure", 
    "Episode 7: Daily Bias & Consolidation", "Episode 8: Institutional Sponsorship", "Episode 9: Power of 3 (Accumulation, Manipulation, Distribution)", 
  ...
  const courseData = episodeTitles.map((title, index) => {
    if (index === 0) {
      return {
        id: `ep${index + 1}`,
        title,
        videoUrl: "https://www.youtube.com/watch?v=yMPS_FuDrs4",
        rawText: "Imagine you are playing a video game where you have to collect coins. In the stock market, the big banks and algorithms are the players, and the 'coins' are called Liquidity. Liquidity is just a fancy word for other people's money. When regular people buy a stock, they get scared it might drop, so they put a 'Stop Loss' order right below the recent lowest point on the chart. The algorithm acts like a giant magnet, pulling the price down just enough to trigger all those stop losses. We call this a Liquidity Sweep.",
        content: (
    ...
    if (index === 1) {
      return {
        id: `ep${index + 1}`,
        title,
        videoUrl: "https://www.youtube.com/watch?v=Ryh2RSjgLgU",
        rawText: "How do we know the algorithm is ready to reverse? We look for a giant footprint called a Market Structure Shift. Imagine someone jumping and stomping as hard as they can, breaking the floorboards. That stomp is displacement. A Market Structure shift happens when price breaks past the last little hill or valley on the chart with massive energy.",
        content: (
          <div className="space-y-6 text-slate-300 leading-relaxed text-lg">
    ...
    if (index === 2) {
      return {
        id: `ep${index + 1}`,
        title,
        videoUrl: "https://www.youtube.com/watch?v=nQfHZ2DEJ8c",
    ...
    if (index === 3) {
      return {
        id: `ep${index + 1}`,
        title,
        videoUrl: "https://www.youtube.com/watch?v=PvI-O9bROeg",
        rawText: "Now you know how to find the gap. But we never jump into a gap blindly. We use the Oliver Velez 200 Simple Moving Average. Think of the 200 SMA as a giant river. If the river is flowing down, you only swim down. If the river is flowing up, you only swim up. Also, wait for an Ignition Candle to push off the river before you jump in.",
        content: (
          <div className="space-y-6 text-slate-300 leading-relaxed text-lg">
    ...
    // Default structure for episodes 5 to 41 to ensure exhaustive array
    return {
      id: `ep${index + 1}`,
      title,
              ...
              {courseData.map((lesson) => {
                ...
                return (
                  <div key={lesson.id} className="bg-slate-900 p-8 rounded-xl border border-slate-800 shadow-xl animate-in fade-in duration-300">
                    ...
                    {/* NEW YOUTUBE LAUNCHER (Bypasses the Grey Error Box) */}
                    <div className="w-full aspect-video bg-gradient-to-br from-slate-900 to-black rounded-xl border border-slate-700 flex flex-col items-center justify-center p-6 text-center relative overflow-hidden group shadow-2xl mb-8">
                      <a href={lesson.videoUrl} target="_blank" rel="noopener noreferrer" className="relative z-10 flex flex-col items-center cursor-pointer">
                        ...
                        <p className="text-sm text-slate-400 mb-6 max-w-md">Click here to open the full lecture securely on YouTube.</p>
                      </a>
                    </div>
